...
export default {
  ...
  methods:{
    ...
    getTimeStrToDay(game_over_timestamp) {
        ...
        let arr = ["鏃?, "涓€", "浜?, "涓?, "鍥?, "浜?, "鍏?];
        let d = arr[date.getDay()]
        return date.getFullYear() + "骞? + (date.getMonth() + 1) + "鏈? + date.getDate() + '鏃? + ' ' + ' ' + '鏄熸湡' + d  + ' ' + "  " + hours + ":" + minutes + ":" + seconds
    },
	times() {
		setInterval(()=>{
	...
	filterTime(time) {
	  ...
	  const H = date.getHours() < 10 ? '0' + date.getHours() : date.getHours() // 灏忔椂
	  const I = date.getMinutes() < 10 ? '0' + date.getMinutes() : date.getMinutes() // 鍒嗛挓
	  const S = date.getSeconds() < 10 ? '0' + date.getSeconds() : date.getSeconds() // 绉?
