...
<div :style='{"border":"2px solid #a4d9b7","padding":"20px","margin":"20px auto 0","borderRadius":"8px","flexWrap":"wrap","background":"#fff","width":"1200px","position":"relative"}'>
    <el-form
      ...
      label-width="200px"
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="澶村儚" v-if="type!='cross' || (type=='cross' && !ro.avatarurl)" prop="avatarurl">
            <file-upload
            tip="鐐瑰嚮涓婁紶澶村儚"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' class="upload" v-else label="澶村儚" prop="avatarurl">
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="璇勮鍐呭" prop="content">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="璇勮鍐呭"
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="鍥炲鍐呭" prop="reply">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="鍥炲鍐呭"
...
<script>
  export default {
    ...
    // 澶氱骇鑱斿姩鍙傛暟
      // 澶氱骇鑱斿姩鍙傛暟
      info(id) {
        this.$http.get('discussjiaotongluxian/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('discussjiaotongluxian/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 璺ㄨ〃璁＄畻
                          ...
                          this.$http.post('discussjiaotongluxian/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '鎿嶄綔鎴愬姛',
                                      type: 'success',
